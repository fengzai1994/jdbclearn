...
/*
    JDBC工具类
    1. 维护一个链接池对象， 维护了一个线程绑定变量的ThreadLocal对象
    2. 对外提供在ThreadLocal中获取链接的方法
    3. 对外提供回收链接的方法，回收过程中，将要回收的链接从ThreadLocal中移除

    注意：工具类仅对外提供共性的功能代码，所以方法均为静态放发
    注意：使用ThreadLocal就是用一个线程在多次数据库操作过程中，使用的是同一个链接
 */
    ...
    public static void release() {
        try {
            ...
            if (connection != null) {
                ...
                threadLocal.remove();
                // 如果开启的事务的手动提交，操作完毕后，归还给连接池之前，要讲事务的自动提交改true
